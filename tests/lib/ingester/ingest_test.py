...
class TestIngest:
    ...
    def test_creates_question_record(self):
        import os
        with Session(Helpers.setup_everything(FixturesLoader.load_q1)[1]) as session:
    ...
    def test_creates_question_with_prints_logs_record(self):
        with Session(Helpers.setup_everything(FixturesLoader.load_q3_with_prints)[1]) as session:
    ...
    def test_creates_question_with_choices_record(self):
        with Session(Helpers.setup_everything(FixturesLoader.load_q3_with_choices)[1]) as session:
            stmt = select(Question).where(Question.internal_id == 'question_three')
            for question in session.scalars(stmt, 0):
